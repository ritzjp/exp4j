...
public class Tokenizer {
    ...
    public Token nextToken() {
        ...
        if (Character.isDigit(ch) || ch == '.') {
            if (lastToken != null) {
                ...
                } else if ((lastToken.getType() != Token.TOKEN_OPERATOR && lastToken.getType() != Token.TOKEN_PARENTHESES_OPEN &&
            ...
            if (lastToken != null &&
                    (lastToken.getType() != Token.TOKEN_OPERATOR && lastToken.getType() != Token.TOKEN_PARENTHESES_OPEN &&
                            lastToken.getType() != Token.TOKEN_FUNCTION && lastToken.getType() != Token.TOKEN_SEPARATOR)) {
                // insert an implicit multiplication token
                lastToken = new OperatorToken(Operators.getBuiltinOperator("*", 2));
                return lastToken;
    ...
    private Token parseFunctionOrVariable() {
        ...
        while (!isEndOfExpression(offset + len - 1) &&
                (isAlphabetic(expression[offset + len - 1]) || Character.isDigit(expression[offset + len - 1]) || expression[offset + len - 1] == '_')) {
    ...
    private Operator getOperator(String symbol) {
        ...
        if (op == null && (symbol.length() == 1 || symbol.length() == 2)) {
            final int argc = (lastToken == null || lastToken.getType() == Token.TOKEN_OPERATOR || lastToken.getType() == Token.TOKEN_PARENTHESES_OPEN || lastToken
                    .getType() == Token.TOKEN_SEPARATOR) ? 1 : 2;
            
            op = Operators.getBuiltinOperator(symbol, argc);
        }
        return op;
    }
    ...
    private Token parseNumberToken(final char firstChar) {
        ...
        while (!isEndOfExpression(offset + len) &&
                isNumeric(expression[offset + len], expression[offset + len - 1] == 'e' || expression[offset + len - 1] == 'E')) {
            len++;
            this.pos++;
        }
